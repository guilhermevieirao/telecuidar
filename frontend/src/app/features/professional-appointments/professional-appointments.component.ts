...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, NgIf, NgFor, RouterLink, FormsModule, BreadcrumbComponent, NotificationsComponent, MobileMenu],
  templateUrl: './professional-appointments.component.html',
  styleUrls: ['./professional-appointments.component.scss']
})
export class ProfessionalAppointmentsComponent implements OnInit {
  ...
  loadAppointments(): void {
    ...
    console.log('Carregando consultas do profissional...');

    this.appointmentService.getMyProfessionalAppointments(true).subscribe({
      next: (appointments) => {
